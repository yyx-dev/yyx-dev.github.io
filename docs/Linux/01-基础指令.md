...
| 路径类型 | 解释                                                         |示例|
| -------- | ------------------------------------------------------------ | -------- |
| 绝对路径 | 从根目录到文件所在目录再加上文件名，具有唯一性 |`/home/yyx/var(--vp-c-bg-soft).c`|
| 相对路径 | 相对当前地址的路径，不以根目录开头不具有唯一性 |`yyx/var(--vp-c-bg-soft).c`、`./a.out`|
...
drwxrwxr-x 2 yyx yyx 4096 Dec 14 06:32 var(--vp-c-bg-soft)
...
<img src="./01-基础指令.assets/文件属性开头的-和d示例.png" style="zoom: 60%;" />
...
<img src="./01-基础指令.assets/touch命令修改文件时间示例.png" style="zoom: 60%;" />
...
```shell
$ rm -f var(--vp-c-bg-soft) # 强制删除属性为只读或称写保护的文件
$ rm -r var(--vp-c-bg-soft) # 递归删除目录及其下所有文件
$ rm -i var(--vp-c-bg-soft) # 删除前逐一询问确认是否删除
```
...
<img src="./01-基础指令.assets/man手册章节大致内容示例.png" style="zoom: 60%;" />
...
```shell
$ cat var(--vp-c-bg-soft).c -n         # 对输出的所有行加行号显示
     1  #include <stdio.h>
     2  int main()
     3  {
     4    printf("hello\n");
     5    return 0;
     6  }
$ cat var(--vp-c-bg-soft).c -b         # 仅对非空的输出行加行号显示
     1  #include <stdio.h>
     2  int main()
     3  {
     4    printf("hello\n");
...
$ cat var(--vp-c-bg-soft).c -b
     ...
     5    return 0;
     6  }
$ cat var(--vp-c-bg-soft).c -s         # 不输出多行空行，仅用一行代替
#include <stdio.h>
int main()
{
...
```sh
$ find . -name var(--vp-c-bg-soft).c # 在当前目录下查找名为var(--vp-c-bg-soft).c的文件
./code/var(--vp-c-bg-soft).c
```
...
$ zip -r code.zip code #压缩目录code为code.zip
  ...
  adding: code/var(--vp-c-bg-soft).c (deflated 10%)
  adding: code/dir/ (stored 0%)
$ unzip code.zip -d .. #解压文件code.zip至上级目录
Archive:  code.zip
   creating: code/
  inflating: code/var(--vp-c-bg-soft).c
   creating: code/dir/
```
