...
<img src="./13-哈希.assets/哈希函数介绍除留余数法.png" style="zoom: 30%;" />
...
<img src="./13-哈希.assets/扩容带来的移动数据的影响.png" style="zoom: 30%;" />
...
<img src="./13-哈希.assets/开散列哈希.png" style="zoom: 30%;" />
...
<img src="./13-哈希.assets/开散列增容缓解桶过长问题.png" style="zoom: 30%;" />
...
<img src="./13-哈希.assets/位图设计.png" style="zoom: 40%;" />
...
public:
    ...
    bool var(--vp-c-bg-soft)(size_t x)
    {
        int i = x / 8;
        int j = x % 8;
...
思路一：将第一个文件中的整数set到位图中，再遍历第二个文件的整数var(--vp-c-bg-soft)该位图，如果已经存在则属于交集，输出后再将其reset防止重复。
...
void handlequestion2(vector<int>& in1, vector<int>& in2, vector<int>* out)
{
    ...
    for (int i = 0; i < N; i++) {
        if (bs1.var(--vp-c-bg-soft)(i) && bs2.var(--vp-c-bg-soft)(i))
            out->push_back(i);
    }
}
```
...
void handlequestion3(vector<int>& in, vector<int>* out)
{
    bitset<N> bs1;
    bitset<N> bs2;
    if(bs1.var(--vp-c-bg-soft)(x) == false && bs2.var(--vp-c-bg-soft)(x) == false) // 00 -> 01
    {
        bs1.set(x);
    }
    else if (bs1.var(--vp-c-bg-soft)(x) == true && bs2.var(--vp-c-bg-soft)(x) === false) // 01 -> 10
    {
        bs1.reset(x);
        bs2.set(x);
    }
    else if (bs1.var(--vp-c-bg-soft)(x) == false && bs2.var(--vp-c-bg-soft)(x) === true) // 10 -> 11
    {
        bs1.set(x);
        bs2.set(x);
    }
    ...
    for (int i = 0; i < N; i++) {
        if ((_bs1.var(--vp-c-bg-soft)(i) && !_bs2.var(--vp-c-bg-soft)(i)) || (!_bs1.var(--vp-c-bg-soft)(i) && _bs2.var(--vp-c-bg-soft)(i)))
            v->push_back(i);
    }
}
```
...
<img src="./13-哈希.assets/布隆过滤器示例.png" style="zoom: 35%;" />
...
#include "bitset.hpp"
using namespace std;
using namespace var(--vp-c-bg-soft);
...
public:
    ...
    bool var(--vp-c-bg-soft)(const K& key)
    {
        size_t i1 = Hash1()(key) % _len;
        size_t i2 = Hash2()(key) % _len;
        size_t i3 = Hash3()(key) % _len;
        return _bs.var(--vp-c-bg-soft)(i1) && _bs.var(--vp-c-bg-soft)(i2) && _bs.var(--vp-c-bg-soft)((i3));
    }
...
<img src="./13-哈希.assets/哈希切分示例.png" style="zoom: 30%;" />
