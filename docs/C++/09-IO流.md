...
//测试C语言二进制读写
void var(--vp-c-bg-soft)C_Write_Bin() {
...
void var(--vp-c-bg-soft)C_Read_Bin() {
	ServerInfo info;
...
//测试C语言字符读写
void var(--vp-c-bg-soft)C_Write_Text() {
	ServerInfo info = { "192.168.1.1",80 };
	FILE* fout = fopen("info.txt", "w");
	fprintf(fout, "%s %d", info._ip, info._port);
	fclose(fout);
}
void var(--vp-c-bg-soft)C_Read_Text() {
	ServerInfo info;
...
<img src="./09-IO流.assets/iostream.png" style="zoom: 60%;" />
...
<img src="./09-IO流.assets/C++标准输入输出重载函数示例.png" style="zoom: 60%;" />
...
<img src="./09-IO流.assets/istream对象支持bool判断.png" style="zoom: 60%;" />
...
```cpp
void var(--vp-c-bg-soft)CPP_Write_Bin() {
	ServerInfo info = { "192.168.1.1",80 };
	ConfigManage con("config.bin");
	con.WriteBin(info);
}
void var(--vp-c-bg-soft)CPP_Read_Bin() {
	ServerInfo info;
	ConfigManage con("config.bin");
	con.ReadBin(info);
}
void var(--vp-c-bg-soft)CPP_Write_Text() {
	ServerInfo info = { "192.168.1.1",80 };
	ConfigManage con("config.bin");
	con.WriteText(info);
}
void var(--vp-c-bg-soft)CPP_Read_Text() {
	ServerInfo info;
	ConfigManage con("config.bin");
	con.ReadText(info);
}
